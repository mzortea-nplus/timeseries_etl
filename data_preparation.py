...
def plot_raw_data(df_raw, config):
    ...
    if len(temp_sensors) > 1:
        print(
            "!!! Warning: the code supports only one thermometer but {len(temp_sensor)} were found. Keeping only first sensor !!!"
        )
    ...
    for sensor_id in struct_sensors:
        ...
        ax1.set_ylabel(
            get_ylabel(sensor_id),
            color=params["colors"]["base_blue"],
            fontsize=FONT_SIZE,
        )
        ax1.tick_params(
            axis="y", labelcolor=params["colors"]["base_blue"], labelsize=FONT_SIZE
        )
...
def run_preparation(
    ...
    # Rename index -> column
    nans_percentage = nans_percentage.rename_axis(
        "sensore"
    ).reset_index()  # rename index
